...
for exp = 1:5  
 ...
 %reorder data dark more Quantity / more quantity hi  
    if (Condition == " Faster" || Condition == " Slower" || Condition == " Rank")
            Ord = [3 1 2 4]%[2 4 3 1];%[3 1 2 4]; %1 & 4 are L+ quantity and 3 & 2 are D+ quantity
 ...
 %used for x axis labels
    ...
    if exp == 1
     ...
     %dark more of the quantity- used for ordering bars - dont need because
     %bars already re-ordered
    ...
    %dark more of the quantity- used for ordering bars
   if (Condition == " Faster" || Condition == " Shorter" || Condition == " Rank")
        Ord2 = [2 1];
   else
        Ord2 = [1 2];
